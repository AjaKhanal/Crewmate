...
public class Mascot {
	...
	private void showPopup(final int x, final int y) {
		...
		// "Reduce to One!" menu item
		final JMenuItem onlyOneMenu = new JMenuItem(
				Main.getInstance().getLanguageBundle().getString("DismissAllOthers"));
		...
		// ghosts.
		com.group_finity.mascot.menu.JLongMenu submenu = new com.group_finity.mascot.menu.JLongMenu(
				Main.getInstance().getLanguageBundle().getString("SetBehaviour"), 30);
						...
						public void actionPerformed(final ActionEvent e) {
							try {
								...
								Main.showError(Main.getInstance().getLanguageBundle()
										.getString("CouldNotSetBehaviourErrorMessage") + "\n" + err.getMessage() + "\n"
										+ Main.getInstance().getLanguageBundle().getString("SeeLogForDetails"));
							}
						}
					});
	...
	public Rectangle getBounds() {
		if (getImage() != null) {
			...
			final Rectangle result = new Rectangle(left, top, getImage().getSize().width * scaling,
					getImage().getSize().height * scaling);
